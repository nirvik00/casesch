...
Failure = namedtuple(
    'Failure', ['line', 'element', 'message', 'role', 'location', 'test'])
...
def _get_unfired_rules(schematron, phase):
    ...
    if schematron_tree is None:
        raise Exception(
            'Schematron is missing schematron attribute. Was schematron created with `store_schematron=True`?')
...
def validate_schematron(schematron, document, result_path=None, phase=None, strict_context=False):
    ...
    if phase is not None:
        ...
        phase_elem = schematron_tree.xpath(
            f'//sch:phase[@id = "{phase}"]', namespaces=SCH_NSMAP)
        if len(phase_elem) == 0:
    ...
    if isinstance(document, str):
        ...
        raise Exception(
            f'Unrecognized type for `document`: {type(document)}. Expected file path, string of xml, or lxml etree instance')
    ...
    for failed_assert in failed_asserts:
        ...
        try:
            ...
            failed_element = document_tree.xpath(
                '/auc:BuildingSync', namespaces=BSYNC_NSMAP)[0]
        readable_xpath = document_tree.getpath(failed_element)
        tag = failed_element.tag.replace(

...
def get_rule_warnings(tree, xpath):
    ...
    try:
        ...
        if not matched and xpath != "/":
            warnings.append(
                f'WARNING: found no matches with exemplary xml for rule\n    context: {xpath}')
        if num_visited > 0:
            warnings.append(
                f'WARNING: rule matches nodes that have already been visited in this pattern: matched and already visited: {num_visited}; matched and unvisited: {num_unvisited}\n    context: {xpath}')
    except Exception as e:
        warnings.append(
            f'WARNING: failed to check rule: {e}\n    context: {xpath}')
    return warnings
...
def add_assert_description(assert_elem, description):
    ...
    for match in matches:
        ...
        inserted_elem = etree.SubElement(assert_elem, qname(
            some_elem.tag), **{k: v for (k, v) in some_elem.items()})
...
def make_pattern_for_testing_contexts(pattern):
    ...
    """
    collected_contexts = {
        rule['context']: True for rule in pattern['rules'] if rule['generate_context_test']}
...
def generate_tests_for_rule_contexts(orig_sch_dict):
    ...
    for phase_idx, phase in enumerate(orig_sch_dict['phases']):
        ...
        for orig_pattern_idx, pattern in enumerate(phase['patterns']):
            ...
            new_sch_dict['phases'][phase_idx]['patterns'].insert(
                prereq_insert_idx, prereq_pattern)
            num_inserted_patterns += 1

    return new_sch_dict


def generate_sch(rows, output_file=None, exemplary_xml_file=None, dry_run=False):
    """
    ...
    # directly reading in app, sending rows from app instead of csv_file
    ...
    for i, row in enumerate(rows):
        ...
        if row['phase title']:
            ...
            if not row['pattern title']:
                # +1 b/c 0-based, +1 b/c csv header
                raise Exception(
                    f'New phase row is missing new pattern title (row {i + 2})')
        ...
        if row['pattern title']:
            ...
            if not row['rule context']:
                # +1 b/c 0-based, +1 b/c csv header
                raise Exception(
                    f'New pattern row is missing new rule context (row {i + 2})')
            current_pattern = new_pattern
            current_phase['patterns'].append(current_pattern)

        # handle new rule
        if row['rule context']:
            generate_context_test = row.get(
                'generate context test', '').lower().strip()
            if generate_context_test not in ['true', 'false']:
                raise Exception(
                    f'Missing required value of "true" or "false" for column "generate context test" in row {i + 2}')
    ...
    etree.SubElement(root, qname('ns'), prefix="auc",
                     uri="http://buildingsync.net/schemas/bedes-auc/2019")

    # used to add patterns at the end of the root element after everything's finished
    collected_patterns = []

    for phase in sch_dict['phases']:
        phase_elem = etree.SubElement(root, qname(
            'phase'), id=phase['id'], see=phase['see'])
        for pattern in phase['patterns']:
            reset_rule_visits()
            etree.SubElement(phase_elem, qname(
                'active'), pattern=pattern['id'])

            pattern_elem = etree.Element(
                qname('pattern'), see=pattern['see'], id=pattern['id'])
            etree.SubElement(pattern_elem, qname(
            ...
            for rule in pattern['rules']:
                ...
                rule_elem = etree.SubElement(
                    pattern_elem, qname('rule'), context=rule['context'])

                for variable in rule['variables']:
                    etree.SubElement(rule_elem, qname(
                        'let'), name=variable['name'], value=variable['value'])

                for assert_ in rule['asserts']:
                    assert_elem = etree.SubElement(rule_elem, qname(
                        'assert'), test=assert_['test'], role=assert_['role'])
    ...
    sch_bytes = etree.tostring(
        root, doctype='<?xml version="1.0" encoding="UTF-8"?>', pretty_print=True)

    str_lines = sch_bytes.decode(encoding="UTF-8")
    return str_lines
